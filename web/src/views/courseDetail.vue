        ...
        <div class="row course-head">
          ...
          <div class="col-sm-6">
            ...
            <div>
              ...
              <p>
                <img
                  v-show="!collectionState.id"
                  v-on:click="courseCollection()"
                  src="../../public/static/image/following.png"
                  style="margin: 5px 0 0 20px"
                />

                <img
                  v-show="collectionState.id"
                  v-on:click="delectCourseCollection()"
                  src="../../public/static/image/followed.png"
                  style="margin: 5px 0 0 20px"
                />
              </p>
            </div>
        ...
        <div class="row">
          ...
          <div class="col-md-9">
            ...
            <ul class="nav nav-tabs">
              ...
              <li class="nav-item">
                <a class="nav-link" href="#chapter" data-toggle="tab"
                  >章节目录</a
                >
              </li>
                ...
                <div v-for="commentItem in courseComment">
                  <div class="comment clearfix">
                    <div class="comment-header">
                      <img
                        class="lecturer-uimg"
                        src="../../public/static/image/header.jpg"
                      />
                    </div>
...
export default {
  ...
  methods: {
    ...
    enroll() {
      ...
      _this.$ajax
        .post(
          process.env.VUE_APP_SERVER + "/business/web/memberCourse/enroll",
          {
    ...
    getEnroll() {
      ...
      _this.$ajax
        .post(
          process.env.VUE_APP_SERVER + "/business/web/memberCourse/get-enroll",
          {
            courseId: _this.course.id,
            memberId: loginMember.id,
          }
      ...
      _this.$ajax
        .get(
          process.env.VUE_APP_SERVER +
            "/business/web/courseComment/comment-list/" +
            _this.id
        )
        .then((response) => {
          let resp = response.data;
    ...
    submitComment() {
      ...
      if (Tool.isEmpty(loginMember)) {
        ...
        if (_this.newComment === null) {
          ...
          _this.$ajax
            .post(
              process.env.VUE_APP_SERVER + "/business/web/courseComment/save",
              {
                courseId: _this.course.id,
                memberId: loginMember.id,
    ...
    courseCollection() {
      ...
      let loginMember = Tool.getLoginMember();
        ...
        /* 发生收藏请求，参数包括：courseId，memberId 
            收藏前需要判断是否满足--未收藏,已登录
            
        */
        if(Tool.isEmpty(_this.collectionState)&&!Tool.isEmpty(loginMember)) {
            _this.$ajax
            .post(
              process.env.VUE_APP_SERVER + "/business/web/courseCollection/save",
              {
                courseId: _this.course.id,
                memberId: loginMember.id,
              }
    ...
    ifCollection() {
      ...
      let loginMember = Tool.getLoginMember();
       ...
       if(Tool.isEmpty(loginMember)) {
        ...
        _this.$ajax
            .post(
              process.env.VUE_APP_SERVER + "/business/web/courseCollection/ifCollection",
              {
            ...
            .then((response) => {
              console.log("查询收藏结果:", response);
              let resp = response.data;
              if (resp.success) {
                _this.collectionState=resp.content|| {}
              } 
            });
       }
    },
    ...
    //取消当前课程的收藏
    delectCourseCollection() {
        let _this =this;
        let loginMember = Tool.getLoginMember();

         _this.$ajax
            .post(
              process.env.VUE_APP_SERVER + "/business/web/courseCollection/delect-collection",
              {
                courseId: _this.$route.query.id,
                memberId: loginMember.id,
              }
